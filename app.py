...
def fmt_yen0(yen: float) -> str:
    """金額を1円単位で表示（Excel の #,##0 と同様・小数なし・3桁カンマ区切り）。"""
...
def runway_snapshot(
    ...
    """
    ランウェイ用のネットバーンは、12ヶ月試算の「月次ネットバーン」の最大値を採用する。
    （入金・支払サイトで月1だけ入出金がゼロの月があり得るため、月1だけで net_burn を決めると常に資金減少なしと誤判定する。）
    """
...
def diagnostic_comment(base: RunwayResult, df_base: pd.DataFrame) -> tuple[str, str, str]:
    ...
    if base.net_burn <= 0:
        ...
        state_parts.append(
            f"毎月おおよそ {fmt_yen0(base.net_burn)} 円ずつ現金が減る試算です（ネットバーン）。"
        )
...
st.caption(
    ...
    "（サイトで月初の入出金がゼロの月だけを見ると、実態より楽観的になるため。）"
)
...
for _col in ("グロスバーン（円／月）", "ネットバーン（円／月）"):
    df_compare_show[_col] = df_compare_show[_col].map(fmt_yen0)
...
for i, (title, dff) in enumerate(scenario_dfs.items()):
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate="月 %{x}<br>%{y:,.0f} 円<extra></extra>",
        )
    )
fig.add_hline(
    y=0,
